...
```
backend/
├── main.py                   # FastAPI 应用入口（含 /api/v1/uploads 静态文件挂载）
...
├── config/                   # 配置模块
│   ├── __init__.py
│   └── settings.py           # 应用配置（secrets 读取）
├── utils/                    # 工具函数
│   ├── __init__.py
│   ├── file_util.py          # 文件处理工具
│   └── doc_util.py           # MongoDB 文档转换工具
└── test/                     # 测试用例
    ├── __init__.py
    ├── conftest.py           # pytest 配置
    ├── plugin_helpers.py     # 插件测试加载工具
    ├── test_db_manager.py
...
> 注：secrets 统一存放于项目根目录 `secrets/`（如 `mongodb_url.txt`、`doubao_api_key.txt`），
> 也可通过 `{NAME}_FILE` 环境变量指向 Docker Secrets。后端 `backend/.secrets/` 已废弃。
...
- 动态加载插件（backend.py + hooks.py 双入口）
- 插件可注册自定义 Key
- 插件可注册自定义 API 路由
- 插件生命周期管理（on_load, on_unload）
- 插件可注册钩子（hook_manager，参数契约：前置 `fn(*业务参数)`，后置 `fn(result, *业务参数)`）

### 6. AI 集成

- 集成豆包 AI 进行语义检索（`POST /api/v1/ai/search`）与自动打标签（`POST /api/v1/ai/auto-tag`）
- API Key 通过 `secrets/doubao_api_key.txt` 或 `DOUBAO_API_KEY` 环境变量配置
- 未配置时接口返回 503，其余功能不受影响
...
AI 配置通过根目录 `secrets/doubao_api_key.txt` 文件或 `DOUBAO_API_KEY` 环境变量提供 API Key：
