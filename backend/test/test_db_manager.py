...
class TestRetryOnConnectionError:
    ...
    @pytest.mark.asyncio
    async def test_retry_decorator_retries_on_connection_failure(self):
        mock_func = AsyncMock(side_effect=[AutoReconnect("disconnected"), AutoReconnect("disconnected"), "success"])
    ...
    @pytest.mark.asyncio
    async def test_retry_decorator_does_not_retry_logic_errors(self):
        ...
        assert mock_func.call_count == 1
        db_manager.reconnect.assert_not_called()
